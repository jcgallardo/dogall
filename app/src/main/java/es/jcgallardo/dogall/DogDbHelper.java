...
public class DogDbHelper extends SQLiteOpenHelper {
    ...
    public Cursor getAllRazaPerros(){
        ...
        Cursor c =  getReadableDatabase()
                .query(
                        DogContract.RazaPerroEntry.TABLE_NAME,
                        null,
                        DogContract.RazaPerroEntry.FOUND + "=1",
                        null,
                        null,
                        null,
                        null);
    ...
    public Dog findDogByNames(ArrayList<String> nombres){
        ...
        for (int i=0; i<nombres.size() && (resultado == null || ( resultado != null && resultado.getCount() <= 0)); i++){
            String nombre = nombres.get(i);
            if (!nombre.equals("terrier")) {
                resultado = getReadableDatabase().query(
                        DogContract.RazaPerroEntry.TABLE_NAME,
                        null,
                        DogContract.RazaPerroEntry.OTHERNAMES + " LIKE ? OR " + DogContract.RazaPerroEntry.NAME + " LIKE ?",
                        new String[]{"%" + nombres.get(i) + "%", "%" + nombres.get(i) + "%"},
                        null,
                        null,
                        null);
                System.out.println("filas: " + resultado.getCount());
            }
        }

        if (resultado.moveToLast()){
            dog = new Dog(resultado);
